...
export default function MapApp() {
  ...
  useEffect(() => { setPage(1); listRef.current?.scrollTo(0, 0); }, [filtered]);
      ...
      <aside className="w-72 flex-shrink-0 bg-white border-r border-gray-200 flex flex-col overflow-hidden shadow-sm">
        ...
        <div className="px-3 py-3 bg-green-700 text-white flex items-center gap-2">
          ...
          <button
            ...
            title="Filtros"
            className={`relative p-1.5 rounded-lg flex-shrink-0 transition-colors ${
        ...
        <div className="flex-1 overflow-y-auto relative" ref={listRef}>
          ...
          {selectedPlace && (
            ...
            >
              <div className="flex items-start justify-between gap-1">
                ...
                <button onClick={() => setSelectedPlace(null)} className="text-gray-400 hover:text-gray-600 flex-shrink-0">✕</button>
              </div>
          ...
          {places.length === 0 ? (
            ...
            <ul>
              {visible.map((p, i) => {
                const isSelected = selectedPlace?.lat === p.lat && selectedPlace?.lng === p.lng && selectedPlace?.nombre === p.nombre;
